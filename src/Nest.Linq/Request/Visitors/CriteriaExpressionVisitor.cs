    ...
    internal abstract class CriteriaExpressionVisitor : ExpressionVisitor
    {
        ...
        protected Expression VisitElasticMethodsMethodCall(MethodCallExpression m)
        {
            switch (m.Method.Name)
            {
                case "ContainsAny":
                    if (m.Arguments.Count == 2)
                        return VisitContains("ContainsAny", m.Arguments[0], m.Arguments[1], true);
                    break;

                case "ContainsAll":
                    if (m.Arguments.Count == 2)
                        return VisitContains("ContainsAll", m.Arguments[0], m.Arguments[1], false);
                    break;
        ...
        Expression VisitContains(string methodName, Expression left, Expression right, bool isAny)
        {
            ...
            if (cm != null)
            {
                ...
                return isAny ? new CriteriaExpression(TermsCriteria.Build(field, cm.MemberExpression.Member, values)) : new CriteriaExpression(new BoolCriteria(values.Select(v => new TermCriteria(field, cm.MemberExpression.Member, v)), null, null));
            }
