...
namespace Nest.Linq.Request.Visitors
{
    ...
    class MemberProjectionExpressionVisitor : ElasticFieldsExpressionVisitor
    {
        ...
        {
            var parameter = Expression.Parameter(typeof(Hit<JObject>), "h");
        ...
        internal static object GetDictionaryValueOrDefault(IDictionary<string, object> dictionary, string key, Type expectedType)
        {
            if (dictionary.TryGetValue(key, out object obj) && obj is JToken token)
            {
